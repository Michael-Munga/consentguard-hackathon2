...
CREATE TABLE IF NOT EXISTS beneficiaries (
  ...
  current_stage TEXT NOT NULL CHECK (current_stage IN ('applied', 'identity_verified', 'consent_requested', 'consent_granted', 'data_processed', 'consent_reviewed'))
);
...
CREATE TABLE IF NOT EXISTS lifecycle_transitions (
  id TEXT PRIMARY KEY,
  beneficiary_id TEXT NOT NULL,
  from_stage TEXT CHECK (from_stage IS NULL OR from_stage IN ('applied', 'identity_verified', 'consent_requested', 'consent_granted', 'data_processed', 'consent_reviewed')),
  to_stage TEXT NOT NULL CHECK (to_stage IN ('applied', 'identity_verified', 'consent_requested', 'consent_granted', 'data_processed', 'consent_reviewed')),
  transitioned_at TEXT NOT NULL,
  is_valid_sequence INTEGER NOT NULL CHECK (is_valid_sequence IN (0, 1)),
  FOREIGN KEY (beneficiary_id) REFERENCES beneficiaries(id) ON DELETE CASCADE
);
...
CREATE INDEX IF NOT EXISTS idx_beneficiaries_stage ON beneficiaries(current_stage);
...
CREATE INDEX IF NOT EXISTS idx_transitions_ben ON lifecycle_transitions(beneficiary_id);
